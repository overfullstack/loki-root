...
fun bool(name: String, value: Boolean?, writer: JsonWriter) =
  with(writer) {
    name(name)
    value?.also(::value) ?: nullValue()
  }

fun string(name: String, value: String?, writer: JsonWriter) =
  with(writer) {
    name(name)
    value?.also(::value) ?: nullValue()
  }

fun <T> list(name: String, list: List<T>?, writer: JsonWriter, block: Consumer<T>) =
  with(writer) {
    name(name)
    list(list, this, block)
  }

fun <T> list(list: List<T>?, writer: JsonWriter, block: Consumer<T>) =
  with(writer) {
    when (list) {
      null -> nullValue()
      else -> {
